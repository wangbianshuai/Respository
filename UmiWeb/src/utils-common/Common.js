...
export function GetDvaState() {
    const s = GetStorage("DvaState");
    return IsNullOrEmpty(s) ? {} : JSON.parse(s);
}

export function SetDvaStateByName(name, state) {
    const dvaState = GetDvaState();
    dvaState[name] = state;
    SetDvaState(dvaState)
}

export function SetDvaState(state) {
    SetStorage("DvaState", JSON.stringify(state));
}

export function GetWebDvaState() {
    const dvaState = GetDvaState();
    const state = { ...dvaState, ...window.InitialState };
    SetDvaState(state);
    return state;
}
...
export function ReplaceDataContent(data, content) {
    ...
    for (let key in data) {
        ...
        content = content.replace(new RegExp(keyValue, "g"), escape(v));
