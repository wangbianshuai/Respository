...
export default class BaseIndex extends Component {
    ...
    GetConfig() {
        ...
        if (!window.PageConfigs[this.Name] || EnvConfig.Env === "local") {
            const name = this.Name.replace("_", "/");
            var url = `/risk/configs/${name}.json`
            if (EnvConfig.Env === "local") url = "/risk/configs/getconfig?name=" + this.Name;
    ...
    IsRiskLook() {
        return !this.RightConfig || this.RightConfig.RightPropertyNames.length === 0
    }
    ...
    InitEventAction() {
        ...
        if (this.props.PageData) {
            ...
            this.OrderStatus2 = this.props.PageData.OrderStatus2;
        ...
        this.EventActions = {
            ...
            GetView: this.GetView.bind(this), OpenPage: this.OpenPage.bind(this), IsRiskLook: this.IsRiskLook.bind(this),
