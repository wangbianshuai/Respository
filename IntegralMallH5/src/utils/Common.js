...
export const DataApiUrl = "http://localhost:50686/api/"
export const ConfigApiUrl = "http://localhost/crmdc/api/"
export const IsDist = false;

//export const DataApiUrl = "http://172.30.32.226:8092/api/";
//export const ConfigApiUrl = "http://172.30.32.226:8092/configs/";
...
export function ReBuildProductList(productList) {
    const productArrayList = [];
    const productArray = [];
    if (IsNullOrEmpty(productList)) return productArrayList;

    productList.forEach((value, key, arr) => {
        if (productArray.length === 2) {
            let [...tmpArray] = productArray;//深拷贝array
            productArrayList.push(tmpArray);
            productArray.length = 0;//清空array，重新追加数据
