...
import styles from "../styles/CardPasswordListView.css"
import CardPasswordItem from "./CardPasswordItem"

export default class CardPasswordListView extends Component {
    constructor(props) {
        super(props)
        ...
        const row = (dataRow) => {
            return (
                ...
                <div styles={{ width: '100%' }} >
                     <CardPasswordItem RowData={dataRow} />
                </div>
            )
        }
    ...
    render() {
        ...
        return (
            <div>
                {dataSource.length === 0 ? (<div style={{ padding: 30, textAlign: 'center' }}>暂时没有订单</div>) : this.renderList(dataSource)}
            </div>
        )
    }
}
