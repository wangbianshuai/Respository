...
namespace MouseSync.Code
{
    ...
    {
        List<int> _PortList { get; set; }
        Action<string> _Receive { get; set; }
        Action<byte[]> _ReceiveBytes { get; set; }
        Dictionary<int, PortUpdService> _PortUpdServiceDictioanry { get; set; }

        ConcurrentDictionary<Guid, List<ReceiveItem>> _ReciveDataDictionary { get; set; }

        public UdpService2(List<int> portList, Action<string> _receive)
        {
            _PortList = portList;
            _Receive = _receive;
            _ReciveDataDictionary = new ConcurrentDictionary<Guid, List<ReceiveItem>>();
            _PortUpdServiceDictioanry = new Dictionary<int, PortUpdService>();
            Load();
        }

        public UdpService2(List<int> portList, Action<byte[]> _receiveBytes)
        {
            _PortList = portList;
        ...
        private void Load()
        {
            try
            {
                _PortList.ForEach(p =>
                {
                    _PortUpdServiceDictioanry.Add(p, new PortUpdService(p, (bs) => SetReceive(bs)));
                });
            }
            catch (Exception ex)
            {
                LoggerProxy.Exception("UdpService2", "Load", ex);
            }
        }
    ...
    public class PortUpdService
    {
        ...
        public PortUpdService(int port, Action<byte[]> _receiveBytes)
        {
            _Port = port;
            _ReceiveBytes = _receiveBytes;
        ...
        private void Load()
        {
            ...
            {
                LoggerProxy.Exception("PortUpdService", "Load", ex);
            }
        }
        ...
        private void Receive()
        {
            ...
            {
                LoggerProxy.Exception("PortUpdService", "Receive", ex);
            }
        }

        private void SetReceive(byte[] content)
        {
            if (_ReceiveBytes != null) _ReceiveBytes(content);
        }
        ...
        public void SendBytes(byte[] data, List<string> ipList)
        {
            Task.Factory.StartNew(() => Send(data, ipList));
        }
...
#if DEBUG
            //debug
            int port = _Port % 2 == 0 ? _Port - 1 : _Port + 1;
        ...
        void SendToIp(byte[] bytes, string ip)
        {
            ...
            {
                LoggerProxy.Exception("PortUpdService", "SendToIp", ex);
            }
        }
    }
}
