...
const getPluginsList = (isDev) => {
    if (isDev) pageConfig.devPageConfigs.forEach(p => { entry[p.name] = p.jsPath; pluginsList.push(getHtmlWebpackPlugin(p)); });
    else pageConfig.pageConfigs.forEach(p => { entry[p.name] = p.jsPath; pluginsList.push(getHtmlWebpackPlugin(p)); });
    return pluginsList;
}
