...
export default class Dialog extends BaseIndex {
    ...
    viewDialogOk(props, action) {
        if (!action.parameters) this.initviewDialogOk(props, action);
        const { property } = props;
        const { dialogView } = action.parameters;

        dialogView.modalDialog.onOkClick(property);
    }

    initviewDialogOk(props, action) {
        const { pageAxis } = props;
        //设置数据视图
        const dialogView = Common.arrayFirst(pageAxis.pageConfig.dialogViews, (f) => f.name === action.dialogView);
    ...
    viewDialogCancel(props, action) {
        if (!action.parameters) this.initviewDialogCancel(props, action);
        const { dialogView } = action.parameters;

        dialogView.modalDialog.onCancelClick();
    }

    initviewDialogCancel(props, action) {
        const { pageAxis } = props;
        //设置数据视图
        const dialogView = Common.arrayFirst(pageAxis.pageConfig.dialogViews, (f) => f.name === action.dialogView);
