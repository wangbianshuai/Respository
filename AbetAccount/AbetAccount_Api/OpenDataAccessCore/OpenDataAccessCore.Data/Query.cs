...
namespace OpenDataAccessCore.Data
{
    public class Query : IQuery
    {
        ...
        private Guid _AppAccountId = Guid.Empty;
        ...
        public IQuery AddAppAccounId(Guid appAccountId)
        {
            _AppAccountId = appAccountId;
            return this;
        }
        ...
        public string ToSql()
        {
            ...
            else if (!TableName.Contains("with(nolock)")) TableName += " with(nolock)";
            ...
            if (_AppAccountId != Guid.Empty)
            {
                var parameter = _ParameterList.Where(w => w.ParameterName == "@AppAccountId").FirstOrDefault();
                if (parameter != null) parameter.Value = _AppAccountId;
                else throw new Exception("查询Sql不符合要求！");
            }
            _FieldSql = "*";
