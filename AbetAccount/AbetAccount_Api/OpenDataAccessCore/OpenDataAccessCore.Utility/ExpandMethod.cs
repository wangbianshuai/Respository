...
namespace OpenDataAccessCore.Utility
{
    public static class ExpandMethod
    {
        ...
        public static string GetStringValueIgnoreCase(this Dictionary<string, object> dict, string propertyName)
        {
            if (dict.ContainsKey(propertyName) && dict[propertyName] != null)
            {
                return dict[propertyName].ToString();
            ...
            {
                var key = dict.Keys.Where(w => w.ToLower().Equals(propertyName.ToLower())).FirstOrDefault();
                if (!string.IsNullOrEmpty(key)) return dict[key].ToString();
            }
            return string.Empty;
        }
