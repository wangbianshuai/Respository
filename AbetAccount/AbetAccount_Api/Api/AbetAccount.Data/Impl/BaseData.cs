...
namespace AbetAccount.Data.Impl
{
    /// <summary>
    /// 数据基类
    /// </summary>
    public class BaseData : EntityAccess
    {
        ...
        protected string GetWhereSql(Entity.Data.QueryInfo queryInfo, List<IDbDataParameter> parameterList)
        {
            ...
            queryInfo.ConditionList.ForEach(c =>
            {
                ...
                else if (!string.IsNullOrEmpty(c.ConditionSql))
                {
                    whereList.Add(c.ConditionSql);
                    if (c.Parameters != null)
                    {
                        foreach (var kvp in c.Parameters)
                        {
                            parameterList.Add(this.InParameter(kvp.Key, kvp.Value));
                        }
                    }
                }
                else
