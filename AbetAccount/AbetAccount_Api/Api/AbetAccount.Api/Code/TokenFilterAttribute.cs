...
namespace AbetAccount.Api.Code
{
    ...
    public class TokenFilterAttribute : ActionFilterAttribute
    {
        ...
        string GetSign(ActionExecutingContext context)
        {
            ...
            string clientId = Code.Request.GetHeadersValue(context.HttpContext.Request, "clientId");
            string userAgent = Code.Request.GetHeadersValue(context.HttpContext.Request, "User-Agent");
            string signKey = "QTQyQUMzMTEtMjhCRC00MTA4LTgwRkMtQ0Y3RkMxRkJENjE1";
            ...
            {
                host = "servicewechat";
                string appId = "wxe5544e928f304ee0";
                if(!referer.Contains(appId)) new TokenException("解析Token失败");
            }
            if (!referer.ToLower().Contains(host.ToLower()) && ! referer.ToLower().Contains("app1.a2china.cn"))
            {
        ...
        bool ParseToken(ActionExecutingContext context, string token)
        {
            ...
            if (controllerName == "KeminGame20210322" && actionName == "GetUserInfo")
            {
                string loginmd5 = "d56b699830e77ba53855679cb1d252db";
            ...
            if (request != null)
            {
                ...
                request.Referer = Code.Request.GetHeadersValue(context.HttpContext.Request, "Referer");
