...
namespace OpenDataAccessCore.Utility
{
    public class Common
    {
        ...
        public static IEnumerable<object> GetObjectList(IEnumerable obj)
        {
        ...
        static void DeleteFile2(string path)
        {
            try
            {
                FileAttributes attr = File.GetAttributes(path);
        ...
        public static void Decompress(string zipPath)
        {
            ...
            string newExtractPath = fileInfo.FullName.Replace(fileInfo.Name, DateTime.Now.ToString("yyyyMMddHHmmssfff") + new Random().Next(100000, 999999));

            ZipFile.ExtractToDirectory(zipPath, newExtractPath);

            string extractPath = fileInfo.FullName.Remove(fileInfo.FullName.Length - fileInfo.Extension.Length);

            string name = fileInfo.Name.Remove(fileInfo.Name.Length - fileInfo.Extension.Length);

            CopyDirectory(newExtractPath + "\\" + name, extractPath);

            DeleteFile(newExtractPath);
        }
        ...
        public static void CopyDirectory(string sourcePath, string targetPath)
        {
            ...
            {
                fileName = string.Format("{0}\\{1}", targetPath, fileInfo.Name);

                if (fileInfo is DirectoryInfo)
                {
                    if (!Directory.Exists(fileName)) Directory.CreateDirectory(fileName);

                    CopyDirectory(fileInfo.FullName, fileName);
                }
                else
                {
                    File.Copy(fileInfo.FullName, fileName, true);
                }
            }
        }
