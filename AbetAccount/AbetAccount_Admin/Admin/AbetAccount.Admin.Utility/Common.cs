...
namespace AbetAccount.Admin.Utility
{
    public class Common
    {
        /// <summary>
        ...
        /// </summary>
        /// <returns></returns>
        public static string SignatureAuth(string data, string key, string value)
        {
            var str = data;
            str += "&" + key + "=" + value;
            var _sign = "signature " + key + "&" + Common.MakeSign(str);
            return _sign;
        }
        /// <summary>
        /// 加签名
        ...
        /// <param name="v"></param>
        /// <returns></returns>
        private static string MakeSign(string v)
        {
            MD5 md = MD5.Create();
            var bys = Encoding.UTF8.GetBytes(v);
            using (md)
            {
                bys = md.ComputeHash(bys);
    ...
    public class Md5Helper
    {
        ...
        public static string Md5Hex(byte[] I_SourceArray)
        {
            string R_Result;
            //byte[] temp = Encoding.UTF8.GetBytes(I_SourceArray);
            sbyte[] temp1 = Md5Helper.ByteArray2SByteArray(I_SourceArray);
