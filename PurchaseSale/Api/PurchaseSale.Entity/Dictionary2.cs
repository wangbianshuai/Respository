...
namespace PurchaseSale.Entity
{
    ...
    public class Dictionary2 : EntityModel, IEntity
    {
        ...
        {
            validateList.Add(this.ValidateExists<Dictionary2>("IsDelete=0 and Name=@Name", "对不起，该名称已存在！"));
        }

        public override void UpdateValidate(List<Func<IValidate, IEntityData, string>> validateList)
        {
            validateList.Add(this.ValidateExists<Dictionary2>("Id=@Id and Name=@Name", "true"));
            validateList.Add(this.ValidateExists<Dictionary2>("IsDelete=0 and Name=@Name", "对不起，该名称已存在！"));
        }
    }
