...
namespace EntityDataService.Entity
{
    public abstract class EntityAccess : IEntityAccess
    {
        ...
        {
            return this.GetQueryByPrimaryKey(this.EntityType, primaryKey);
        }

        private IQuery GetQueryByPrimaryKey(EntityType entityType, object primaryKey)
        {
            IQuery query = new Query(entityType.TableName);
            Property primaryKeyProperty = entityType.GetProperty(entityType.PrimaryKey);
            if (primaryKeyProperty != null)
            {
                primaryKeyProperty.Value = DataCommon.ChangeType(primaryKey, primaryKeyProperty.Type);
        ...
        public bool InsertEntity(EntityType entityType, IEntityData entityData, out object primaryKey, IDbTransaction trans = null)
        {
            ...
            for (int i = 0; i < entityData.Count; i++)
            {
                ...
                if (property != null)
                {
                    property.Value = DataCommon.ChangeType(entityData.GetValue(propertyName), property.Type);
            ...
            bool blSucceed = this.CurrentDataBase.ExecSqlNonQuery(sqlText, parameterList, trans);
        ...
        public bool UpdateEntityByPrimaryKey(EntityType entityType, object primaryKey, IEntityData entityData, IDbTransaction trans = null)
        {
            IQuery query = this.GetQueryByPrimaryKey(entityType, primaryKey);
            if (query != null)
            {
                return this.UpdateEntity(entityType, query, entityData, trans);
            }
            return false;
        }
        ...
        public bool UpdateEntity(EntityType entityType, IQuery query, IEntityData entityData, IDbTransaction trans = null)
        {
            ...
            for (int i = 0; i < entityData.Count; i++)
            {
                ...
                {
                    property.Value = Data.DataCommon.ChangeType(entityData.GetValue(propertyName), property.Type);
                    parameterList.Add(this.GetInParameter(property));
