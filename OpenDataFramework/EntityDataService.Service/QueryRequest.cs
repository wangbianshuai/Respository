    ...
    public class QueryRequest
    {
        ...
        private void ParseFilter(string filter, List<string> conditionList)
        {
            ...
            if (logicIndex > 0)
            {
                ...
                if (propertyValue.Trim() != "@" + property.Name)
                {
                    ...
                    property.Value = DataCommon.ChangeType(propertyValue, property.Type);
                    if (this.FilterParamterList == null)
                    {
        ...
        private Property GetPrimaryKeyProperty()
        {
            ...
            if (startIndex >= 0)
            {
                ...
                if (property != null)
                {
                    property.Value = DataCommon.ChangeType(primeryKeyString, property.Type);
                    return property;
                }
            }
            return null;
        ...
        private List<IDbDataParameter> GetFileterParameterList()
        {
            if (this.QueryString.AllKeys != null)
            {
                ...
                if (propertyNameList != null)
                {
                    ...
                    {
                        property.Value = DataCommon.ChangeType(this.GetParameterValue("@" + property.Name), property.Type);
            ...
            this.QueryInfo.WhereFields.ForEach(whereField =>
            {
                ...
                if (!string.IsNullOrEmpty(whereField.Value))
                {
                    if (whereField.OperateLogic != "in" && whereField.OperateLogic != "like")
                    {
                        try
                        {
                            whereField.ObjValue = DataCommon.ChangeType(whereField.Value, whereField.Type);
                        }
                        catch
                        {
                    ...
                    else if (whereField.OperateLogic == "in")
                    {
                        ...
                        {
                            this.QueryInfo.ParameterList.Add(this._EntityAccess.CurrentDataBase.InParameter(whereField.ParameterName + valueList.IndexOf(value).ToString(), DataCommon.ChangeType(value, whereField.Type)));
                        });
                    }
                    else
                    {
        ...
        private string GetEntityPropertySql(string sql)
        {
            ...
            if (fields.Exists(e => e == "RowVersion"))
            {
                fieldSql += ",RowVersion";
            }
            return fieldSql;
