    ...
    public class EntityRequest : EntityAccess, IEntityAccess, IEntityRequest
    {
        ...
        public bool JudgeIsEdit(EntityType entityType, IEntityData newEntityData, IEntityData oldEntityData)
        {
            ...
            foreach (var d in data)
            {
                ...
                else if (d.b.Value != null && !d.b.Value.Equals(DataCommon.ChangeType(d.a.Value, d.b.Value.GetType())))
                {
                    blEdit = true;
                }
                if (blEdit)
        ...
        public object Select()
        {
            ...
            else if (_QueryRequest.IsData && action != "Excel")
            {
                if (string.IsNullOrEmpty(this._QueryRequest.QueryInfo.ProcName))
                {
                    ...
                    sb.AppendFormat(" from {0} {1} {2}) a", _Request.Entity.TableName + " t", query.ToWhereSql(), query.ToGroupSql());
            ...
            else if (action == "Excel")
            {
                if (string.IsNullOrEmpty(this._QueryRequest.QueryInfo.ProcName))
                {
                    ...
                    if (!string.IsNullOrEmpty(groupSql.Trim()))
                    {
                        ...
                        sb.Append("select ");
                        sb.Append(string.IsNullOrEmpty(query.ToSelectSql().Trim()) ? "t.*" : query.ToSelectSql());
                        sb.AppendFormat(" from {0} {1} {2} {3}", _Request.Entity.TableName + " t", query.ToWhereSql(), groupSql, string.IsNullOrEmpty(query.ToOrderBySql().Trim()) ? "order by " + _Request.Entity.PrimaryKey : query.ToOrderBySql());
                        query.SetSql(sb.ToString());
                    }
        ...
        {
            return value == null ? this.CurrentDataBase.InParameter(parameterName, DBNull.Value) : this.CurrentDataBase.InParameter(parameterName, value);
        }
        ...
        private Dictionary<string, object> ExcelExport(List<IEntityData> entityDataList, string title, string entityName)
        {
            ...
            Dictionary<string, object> dict = new Dictionary<string, object>();
            dict.Add("FileName", Utility.FileHelper.Encrypt(fileName));
            return dict;
        }
        ...
        private List<IDbDataParameter> GetFileterParameterList(IEntityData entityData, EntityType entityType, string filter)
        {
            ...
            if (propertyList.Count > 0)
            {
                ...
                {
                    property.Value = DataCommon.ChangeType(entityData.GetValue(property.Name), property.Type);
        ...
        public string InsertValidateUnique<T>(IEntityData entityData, string propertyName, string message) where T : IEntity
        {
            ...
            if (entityType != null)
            {
                ...
                if (property != null)
                {
                    object propertyValue = DataCommon.ChangeType(entityData.GetValue(property.Name), property.Type);
                    IQuery query = new Query(entityType.TableName, entityType.Name);
                    property.Value = propertyValue;
                    List<IDbDataParameter> parameterList = new List<IDbDataParameter>()
                    {
        ...
        public string UpdateValidateUnique<T>(IEntityData entityData, string propertyName, string message) where T : IEntity
        {
            ...
            if (entityType != null)
            {
                ...
                if (property != null)
                {
                    ...
                    IQuery query = new Query(entityType.TableName, entityType.Name);
                    property.Value = propertyValue;
                    primaryKeyProperty.Value = DataCommon.ChangeType(entityData.GetValue(primaryKeyProperty.Name), primaryKeyProperty.Type);
                    List<IDbDataParameter> parameterList = new List<IDbDataParameter>()
                    {
                        this.GetInParameter(property),
