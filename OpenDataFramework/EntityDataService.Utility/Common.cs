...
namespace EntityDataService.Utility
{
    public class Common
    {
        ...
        public static DateTime GetDateValue(string value)
        {
            ...
            double dDate = 0;
            if (value.IndexOf("-") < 0 && double.TryParse(value, out dDate))
            {
        ...
        public static List<List<T>> ListToBatchList<T>(List<T> dataList, int num)
        {
            List<List<T>> batchList = new List<List<T>>();
            List<T> list = null;

            int iCount = dataList.Count % num == 0 ? dataList.Count / num : dataList.Count / num + 1;
            for (var i = 1; i <= iCount; i++)
            {
                list = new List<T>();
                for (int j = (i - 1) * num; j < i * num; j++)
                {
                    if (j < dataList.Count) list.Add(dataList[j]);
                }
                batchList.Add(list);
            }
