...
class ThirtyTender extends BaseIndex {
    ...
    GetTitle() {
        return "新元宝（新手专享）-加入记录-我的新新贷";
    }

    GetCssList() {
        return ["/build/mods/user/bonds/thirtytender/_.css"];
    }

    GetJsList() {
        return ["/build/mods/user/bonds/thirtytender/_.js"];
    }

    //服务器渲染加载数据
    static LoadData(ctx, app) {
        return BaseIndex.Dispatch(app._store.dispatch, "PageView", "GetBondsThirtyTender");
    }
    ...
    render() {
        ...
        return (
            <div id="J_wrapBody">
                ...
                <a href="javascript:void(0);">加入记录</a> &gt; <a href="javascript:void(0);">新元宝(新手专享)</a>
                        </div>
                    </div>
                </div>
                <div className="info-contaner clearfix">
                    <Menu MenuName1="tender" MenuName2="thirtytender" MenuStatus={MenuStatus} />

                    <div className="g-right">
                        <div className="m-con-wrap">
                            <div className="m-con-hd user-title" id='J_tabs'>
                                <li className='active'><a href="#">债权列表</a></li>
                                <li className='fr back'><a href="/usercenter/tender/thirtytender.html" className='return'>返回</a></li>
                            </div>
...
function mapStateToProps(state, ownProps) {
    return {
        PageData: state.PageView.BondsThirtyTender
    };
}
