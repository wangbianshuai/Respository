...
export default class MonthGoldInvest extends BaseIndex {
    constructor(props) {
        super(props);
    ...
    InitProperty(investProduct) {
        ...
        this.setState({ income: this.ComputeIncome(lowestTender), IsEnable: status === 2 && meetTermOfPurchase, limitMoney: maxAmount, investMoney: this.lowestTender });
    }
    ...
    ReceiveAuthorizedQuota(data) {
        ...
        this.DialogInvestmentElement.Show({
            ...
            url: '/commpd/agree/monthgold_agree_' + id + '.html?productSign=YJDJ',
            agreement: '月进斗金'
        });
    }
    ...
    SelectMoney(amount, IsEnable) {
        if (!(this.maxAmount > amount && IsEnable)) return;
    ...
    MinusPlus(blMinus) {
        ...
        const { step, remAccount, usable, lowestTender, maxAmount } = this;
    ...
    render() {
        ...
        const { investMoney, errorTip, riskChecked, limitMoney, IsEnable, income } = this.state;

        return (
            <div className='detail-focus'>

                {IsLogin
                    ? <p>账户余额：{Common.ToCurrency(InvestProduct.usable)} <a href='/usercenter/recharge.html' id="recharge">充值</a></p>
                    : <p>账户余额：<a className='tologin' href='/user/ilogin.html'>登录</a>后可见</p>
                }
                <p>加入金额：（{InvestProduct.step}的整数倍递增）</p>
                <div className='money' id="J_money">
                    <button type='button' value='500' onClick={this.SelectMoney(500, IsEnable)} className={InvestProduct.maxAmount > 500 && IsEnable ? '' : 'disable'}>500元</button>
                    <button type='button' value='800' onClick={this.SelectMoney(800, IsEnable)} className={InvestProduct.maxAmount > 800 && IsEnable ? '' : 'disable'}>800元</button>
                    <button type='button' value='1000' onClick={this.SelectMoney(1000, IsEnable)} className={InvestProduct.maxAmount > 1000 && IsEnable ? '' : 'disable'}>1000元</button>
                    <button type='button' value='2000' onClick={this.SelectMoney(2000, IsEnable)} className={InvestProduct.maxAmount > 2000 && IsEnable ? '' : 'disable'}>2000元</button>
                    <button type='button' value='3000' onClick={this.SelectMoney(3000, IsEnable)} className={InvestProduct.maxAmount > 3000 && IsEnable ? '' : 'disable'}>3000元</button>
                </div>
                ...
                <p>历史收益：<span className="month-color">{income}</span>元</p>
                {IsLogin && <p>您的个人剩余可投额度：<span className='limit month-color'>{Common.ToCurrency(limitMoney)}</span>元</p>}
                <p><i></i>资金由存管银行全程监控</p>
