		...
		sliderBox:function(parma){
			...
			if(_dataType == 'json'){
				$.each(_data,function(index,item){
                    ...
                    if(item.linkurl == 'javascript:void(0);' || item.linkurl == ''){
                    	_this.find('ul').append('<li><a class="ga-click dmp-click" eventType = "jump" dev_id="'+ banner_id +'" target_id=" '+ item.imgId +'" textHref="'+ item.linkurl +'" dmp_text="'+ item.title +'" ga-commnd="send,ec:addPromo,ec:setAction" ga-ec-addPromo="{\'name\':\' '+ item.title +'\'}" ga-ec-setAction-command="promo_click" ga-hitType="event" ga-category="增强电商" ga-action="站内广告点击" ga-label=" '+ item.title +'" href="javascript:void(0);"><img src="' + item.imgurl + '" /></a><h2>' + item.title + '</h2><span>' + item.content + '</span></li>');
                    }else{
