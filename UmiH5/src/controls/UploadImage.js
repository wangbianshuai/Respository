...
class UploadImage extends BaseIndex {
    ...
    OnChange(e) {
        ...
        if (!/image/.test(file.type)) { this.ShowInfo("请选择图片"); return; }

        const reg = /\.(jpg|jpeg|png)$/i;
        if (!reg.test(file.name)) { this.ShowInfo("请选择jpg、jpeg、png格式图片"); return; }

        // 大于 10MB
        if (file && file.size > 1024 * 1024 * 10) { this.ShowInfo('请使用小于10MB的图片'); return }
