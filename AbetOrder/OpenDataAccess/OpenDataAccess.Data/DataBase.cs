...
namespace OpenDataAccess.Data
{
    ...
    internal abstract class DataBase : IDataBase
    {
        ...
        private IDbCommand GetCommand(string commandText, bool blSql, IDbTransaction trans = null)
        {
            ...
            {
                _IDBCommand.Connection = ConnectionPool.GetConnection(this.ConnectionString, ClientType);
            }
        ...
        private DataSet FillDataSet(IDbCommand command)
        {
            ...
            {
                _IDBDataAdapter = new MySqlDataAdapter((MySqlCommand)command);
            }
            try
            {
        ...
        {
            IDbConnection conn = ConnectionPool.GetConnection(connectionString, ClientType);
        ...
        public IDbDataParameter Parameter(string parameterName, ParameterDirection paramDirection, object value)
        {
            ...
            if (this.ClientType == ServerClient.OracleClient || this.ClientType == ServerClient.MySqlClient)
            {
                if (value is Guid)
                {
                ...
                if (this.ClientType == ServerClient.MySqlClient) param = new MySqlParameter(parameterName, value);
                else param = new OracleParameter(parameterName, value);
            }
            else
            {
